...
const activityExpectedStructure = {
    ...
    //date: expect.stringMatching(//) // regex will match only string with format YYYY-MM-DD
    notes: expect.any(String),
    time: expect.any(Number),
    commissionName: expect.any(String),
    manufacturingName: expect.any(String)
};
const commissionExpectedStructure = { name: expect.any(String) };
const manufactureExpectedStructure = { name: expect.any(String) };
...
describe('API integration test', () => {
    ...
    describe("[POST] /api/activities", () => {
        ...
        it('returns an error for commission name and/or manufacture name not found', async () => {
            const res = await request(app)
                .post('/api/activities')
                .send({
                    commissionName: "Not found",
                    manufacturingName: "Not found",
                    date: "2021-12-01",
                    notes: "",
                    time: 1
                })
    ...
    describe("[GET] /api/activities/:id", () => {
        ...
        it('returns an error for activity not found', async () => {
            ...
            expect(res.body).toMatchObject({message: "Cannot find Activity with id=-1."})
        })
    })

    describe("[GET] /api/activities/byCommissionName?commissionName={value}", () => {
        it('correctly returns an array of activities', async () => {
            const res = await request(app).get('/api/activities/byCommissionName?commissionName=Banchi%20DISI')
            expect(res.statusCode).toBe(200)
            for (const activity of res.body) {
                expect(activity).toMatchObject(activityExpectedStructure)
            }
        ...
        it('returns an error for commissionName not found', async () => {
            const res = await request(app).get('/api/activities/byCommissionName?commissionName=-1')
            expect(res.statusCode).toBe(404);
            expect(res.body).toMatchObject({message: "Cannot find Commission with name=-1."})
        })
    })
